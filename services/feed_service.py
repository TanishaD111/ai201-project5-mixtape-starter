...
# BUG #2 FIX: "Friends Listening Now" should show only friends who have listened
# TODAY. The original code used a rolling 24-hour window (cutoff = now - 24h), so a
# friend's late-night listen kept appearing until the same clock time the next day
# (e.g. an 11pm listen still showed at 9am the next morning). The cutoff is now the
# start of the current calendar day (UTC), computed per-request in the function below.
