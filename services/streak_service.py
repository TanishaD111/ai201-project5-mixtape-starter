...
def update_listening_streak(user: User, now: datetime) -> None:
    ...
    if days_since_last == 0:
        ...
        return
    # BUG #1 FIX: the extra clause `and today.weekday() != 6` treated Sunday (weekday()==6)
    # as a non-consecutive day, so a Sat->Sun listen fell through to the reset below.
    # A consecutive day is consecutive regardless of weekday.
